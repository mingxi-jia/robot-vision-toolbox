...
class Renderer:
    ...
    def __init__(self, cfg: CfgNode, faces: np.array):
        ...
        # Cache for OffscreenRenderer (reuse instead of recreating every frame)
        # Safe because rendering is single-threaded in detect_hand_pipeline_batch
        self._offscreen_renderer_cache = {}  # Key: (width, height)
    ...
    def render_rgba(
        ...
        # Use cached renderer to avoid repeated creation/destruction (2-3x speedup)
        render_key = (render_res[0], render_res[1])
        if render_key not in self._offscreen_renderer_cache:
            self._offscreen_renderer_cache[render_key] = pyrender.OffscreenRenderer(
                viewport_width=render_res[0],
                viewport_height=render_res[1],
                point_size=1.0
            )
        renderer = self._offscreen_renderer_cache[render_key]
        # material = pyrender.MetallicRoughnessMaterial(
        #     metallicFactor=0.0,
        #     alphaMode='OPAQUE',
        #     baseColorFactor=(*mesh_base_color, 1.0))
        ...
        color, rend_depth = renderer.render(scene, flags=pyrender.RenderFlags.RGBA)
        color = color.astype(np.float32) / 255.0
        # renderer.delete()  # Commented out: using cached renderer, will be cleaned up on process exit
