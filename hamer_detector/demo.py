...
def detect_hand(args):
    ...
    for img_path in img_paths:
        ...
        if args.full_frame and len(all_verts) > 0:
            ...
            # correct translation matrix to match realsense camera
            # Load mesh using trimesh
            hamer_output_path = os.path.join(args.out_folder, f'{img_fn}.obj')
            mesh = trimesh.load(hamer_output_path, process=False)
            hamer_vertices = np.array(mesh.vertices)
